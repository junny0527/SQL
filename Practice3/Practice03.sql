...
where em.department_id = de.department_id(+) and
	  em.job_id = jo.job_id 
order by employee_id asc;
--팁!!직원이 없는 부서를 나타낼 땐(EMP) 부서가 없는 직원을 나타낼 땐(DEPT)
...
where co.country_id = lo.country_id and
...
select 
        em.employee_id as 사번, 
        em.first_name || ' ' || last_name "이름(풀네임)",
        em.job_id 업무아이디, 
	    start_date 시작일, 
        end_date 종료일
from employees em, job_history jo
where jo.employee_id = em.employee_id and
	  jo.job_id = 'AC_ACCOUNT';
