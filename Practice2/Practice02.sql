...
SELECT
    count(manager_id) as "haveMngCnt" 
FROM employees;

--문제2. 
--직원중에 최고임금(salary)과  최저임금을 “최고임금, 
--“최저임금”프로젝션 타이틀로 함께 출력해 보세요. 두 임금의 차이는 얼마인가요? 
...
SELECT
    max(to_char(hire_date,'YYYY"년"MM"월"DD"일"' )) as 마지막입사일
FROM employees;
...
--출력하고 정렬순서는 최저임금 내림차순, 평균임금(소수점 반올림),오름차순 순입니다.
...
select 
    min(to_char(hire_date,'YYYY-MM-DD day')) as 가장_오래_일한_직원
from employees;
